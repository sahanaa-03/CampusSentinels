...
def run_yolo(event=None):
    ...
    extract_number_plate(results[0].boxes, results[0].names,
                         annotated_image_rgb)  # Pass bounding boxes, names, and annotated image
...
def extract_number_plate(boxes, names, annotated_image):
    ...
    for i, box in enumerate(boxes):
        ...
        if label == "NumberPlate":  # Check if the label is "NumberPlate"
            ...
            enlarged_image = cv2.resize(cropped_image, None, fx=3, fy=3,
                                        interpolation=cv2.INTER_CUBIC)  # Enlarging the image by a factor of 2
            ...
            shadow_removed = cv2.morphologyEx(gray, cv2.MORPH_CLOSE,
                                              kernel)  # Closing operation to remove small shadows
